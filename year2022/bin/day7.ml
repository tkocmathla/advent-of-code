type tree_node = { name : string; size : int; parent : tree_node option; kids : tree_node list; }

let rec total_size node = List.fold_left (fun size kid -> size + total_size kid) node.size node.kids
let rec to_root node = match node.parent with Some p -> to_root p | None -> node
let update_parent_kids kid =
  match kid.parent with
  | Some p -> { p with kids = List.map (fun k -> if k.name = kid.name then kid else k) p.kids; }
  | None -> failwith "err"
...
let () =
  let tree = Arg.read_arg "data/day7.txt" |> Array.to_list |> to_tree in

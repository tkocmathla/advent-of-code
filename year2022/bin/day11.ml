...
let parse_monkey f seq m =
  ...
  let inspect =
    ...
    | _ -> failwith "bad op"
  in
  let target_fun value = if value mod div = 0 then id_true else id_false in
  ({ id; inspect; target = target_fun }, M.add id (0, items) m)
...
let score m =
  ...
  match counts with a :: b :: _ -> a * b | _ -> 0

let solve f n seq =
  let _, monkeys, m =
    Seq.iterate (parse_monkeys f) (seq, [], M.empty) |> Seq.drop 8 |> Seq.uncons
    |> function | None -> failwith "derp" | Some (x, _) -> x
  in
  Seq.iterate round (Seq.cycle (List.to_seq (List.rev monkeys)), m)
  |> Seq.drop n |> Seq.uncons
